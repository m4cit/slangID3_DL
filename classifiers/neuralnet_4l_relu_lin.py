...
##############################################################################################
class NeuralNet_4l_relu_lin(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer_1 = nn.Linear(in_features=input_features, out_features=hidden_size)
        self.relu_1 = nn.Dropout(p=0.5, inplace=False)
        ...
        self.relu_3 = nn.ReLU()
        self.layer_4 = nn.Linear(in_features=hidden_size, out_features=output_size)
    # forward pass method
    def forward(self, x):
...
def dm_nn_4l_relu_lin():
    try:
        # loading trained model
        loaded_model = NeuralNet_4l_relu_lin()
        loaded_model.load_state_dict(torch.load('./classifiers/models/NeuralNet_4l_relu_lin.pth'))
        ...
        f1_and_pred_list = ['NeuralNet_4l_lin:', f'*{f1_score}', f'*{errors}', '']
        ################
...
def predictor_nn_4l_relu_lin(input):
    try:
        # loading trained model
        loaded_model = NeuralNet_4l_relu_lin()
        loaded_model.load_state_dict(torch.load('./classifiers/models/NeuralNet_4l_relu_lin.pth'))
        ...
        # evaluation
        preds = []
        preds.append(f'NeuralNet_4l_lin:\n Phrase:\t         {input[0]}')
