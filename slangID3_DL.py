...
if __name__ == '__main__':
    ...
    # running all models
    def predictor_all(input, stats_nn_1):
        if stats_nn_1 == 0:
    ...
    ################################################################################################################################
    predict_button = ctk.CTkButton(root, command=lambda: predictor_all([phrase_entry.get("1.0",ctk.END)],nn_checkbox_1.get()), text="Predict", font=("Calibri", 22,"bold"), text_color="white", fg_color="#f76700", hover_color="#eeb000", border_width=2, border_color="#e4e4e4", border_spacing=10)
    predict_button.grid(row=1, rowspan=1, column=0, padx=15, pady=10, sticky="nsw")

    train_button = ctk.CTkButton(root, command=lambda: train_all(nn_checkbox_1.get()), text="Train", font=("Calibri", 22,"bold"), text_color="white", fg_color="#f76700", hover_color="#eeb000", border_width=2, border_color="#e4e4e4", border_spacing=10)
    ...
    # select_all_checkbox = ctk.CTkCheckBox(checkbox_frame, command=lambda: (nn_checkbox_1.toggle()), text="All models", font=("Calibri", 20,"bold"), text_color="white", border_color="red", corner_radius=10, fg_color="#b6c2fe", hover_color="#99c8fc")
